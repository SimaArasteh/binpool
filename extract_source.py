...
def download_file_with_wget(url, save_path):
    try:
        ...
        subprocess.run(['dget', '-P', save_path, url], check=True)
...
def download_source(url, save_path):
    try:
        # Ensure the save_path exists
        
        # Change directory to the save_path
        os.chdir(save_path)

        # Build the dget command
        command = ["dget", "-u", "--insecure", url]

        # Run the dget command
        subprocess.run(command, check=True)
        print(f"Source downloaded successfully to {save_path}")
        previous_directory = "/workspaces/binpool/"
        os.chdir(previous_directory)

    except subprocess.CalledProcessError as e:
        print(f"Error while downloading the source: {e}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    for index, row in result_df.iterrows():
        ...
        if final_url != "":
            print(final_url)
            cve_sources[cve].append(final_url)

    
    joblib.dump(cve_sources, "cve_sources_links.pkl")'''

    '''cve_sources = joblib.load("cve_sources_links.pkl")
    for cve in cve_sources:
        print(cve)
        if cve == 'CVE-2014-8241':
            save_path = "CVE_Directories/"+cve
            source_link = cve_sources[cve][0]
            print(source_link)
            download_source(source_link, save_path)'''
